...
# Scope/verifica
1. Acquisisci lock PH e fai un solo fetch/pull fast-forward. Parti dal `main` corrente; niente branch Places da integrare.
...
3. Durante implementazione usa compile/test **mirati** ai file/moduli toccati. Test richiesti: grouping/collapse/no-chip, paging, People bounded, fatigue aggregate, handoff senza doppia query, selection/title/min-2/recreation/persistenza. Nello stesso batch test includi anche l'esistente `CheckInAccuracyPolicyTest` per verificare il fix Places già integrato; **nessun gate/build/QA Places separato**.
4. Fai **un solo host gate aggregato quando l'implementazione è pronta**: test mirati necessari + `:app:assembleQa --no-configuration-cache`; non usare root `./gradlew check` come inner loop. Se il gate fallisce, leggi in una volta tutti gli errori del leaf task/modulo, correggili in batch, rilancia solo quel leaf task e poi fai un solo host gate finale. Non ripetere l'intera build dopo ogni singolo lint/compile fix.
5. Solo dopo host PASS, una sola QA isolata emulator/TCL su intervallo noto + salvataggio subset, riusando l'APK appena costruito. Niente QA Places se non emerge un failure concreto.
6. Commit/push PH; non cambiare `version.txt`. Stop a PASS; side issue non bloccanti solo segnalati.

# Stop
PASS = feature Cerca/Composer + test mirati + host gate + una QA subset. Dopo PASS non cercare nuovi colli di bottiglia né creare gate successivi per questa fase.
