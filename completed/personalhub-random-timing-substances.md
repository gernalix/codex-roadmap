...
1. Acquisisci lock PH; un solo fetch/pull fast-forward. Implementa prima, senza build intermedie salvo errore di compilazione inevitabile.
2. **Una sola invocazione host finale:** `./gradlew check :app:assembleQa --no-configuration-cache`. Deve coprire sia le nuove regressioni della fase sia i fix audit già pushati; non rilanciare test PASS separatamente.
...
# Stop
PASS = feature fase 1 + fix audit già esistenti coperti dallo stesso host gate/QA + cleanup. Side issue non bloccanti: segnala senza investigare. **Nessun audit successivo e nessun nuovo prompt di verifica per questa fase.**
...
Output ≤8 righe: RESULT, picker, random timer/alerts, SinceWhen, UI/People, audit regressions, test/QA, SHA/blocker.