...
Usa `origin/main` corrente dopo i gate token per-prompt; non assumere vecchi SHA. Preserva integralmente `codex_task_costs.py`/`tests/test_task_costs.py`, `codex_prompt_cost_query.py`/`tests/test_prompt_cost_query.py`, `codex_task_costs_incremental.py`/`tests/test_task_costs_incremental.py` e `systemd/codex-session-archive.service`. In particolare non regredire: confine nativo `task_complete`; `completion_state`; esclusione default `eof_incomplete`; query SQLite `mode=ro`; refresh `--refresh-prompt`; chiusure esplicite con `closing`; `cost_source_state`; indicizzazione automatica basata sugli hash di `archive.sqlite`; secondo `ExecStart` della unit su `codex_task_costs_incremental.py` e non sul full builder. Questi file non vanno modificati salvo failure direttamente causata dal lifecycle SQLite.
