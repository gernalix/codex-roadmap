...
1. Quando port + schema + leaf test sono verdi, unico gate campagna pre-bump: temporal/provider WordPulse real-storage, regressioni mirate `381527`+`742913`, `CheckInAccuracyPolicyTest`, `HubActivityRegisterTest`, architecture gate. Se emerge un nuovo failure, leaf fix e **una sola** ripetizione del gate.
...
# Stop
PASS = parity semantica con `wordpulse@ca238d26` + fatigue temporal provider provato su storage reale + migration graph/storici/startup fail-safe + regressioni campagna + unico bump + stesso hash APK verificato/installato/consegnato.
...
Output ≤9 righe: RESULT, WordPulse parity+fatigue provider, schema/grafo, historical/startup, campaign gates, version/APK/hash, Pixel+delivery, SHA PH, blocker.
