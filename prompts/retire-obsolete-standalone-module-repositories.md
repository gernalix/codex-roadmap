...
`gernalix/luoghi-app` NON è incluso in questo task.
...
Per ciascuno dei quattro target, in batch:
...
Se uno dei quattro contiene lavoro locale non committato o commit non pushati, NON cancellarlo: `BLOCKED` con path+stato minimo e STOP prima di qualsiasi delete remoto.

# Esecuzione
Se tutti i gate sono PASS:
1. usa `gh`/GitHub API, non Chrome, per eliminare esattamente i quattro repository remoti;
2. verifica una sola volta che ciascuno restituisca not-found/non-accessibile come repo remoto;
3. elimina esattamente le quattro directory locali risolte, con guard `realpath` che imponga il prefisso `/home/daniele/projects/`; nessun glob e nessun `find -delete`;
...
6. verifica solo riferimenti operativi esatti ai quattro slug in `github-autosync`/config runtime. Rimuovi allowlist/path statici che causerebbero errori `missing/no_upstream`; non modificare documentazione storica o report completati;
7. esegui i test mirati soltanto dei file/config effettivamente modificati.

# Non-goal
- non cancellare `luoghi-app`, `PersonalHub` o altri repo simili;
...
# Acceptance
PASS solo se:
- i quattro repo GitHub indicati non esistono più;
- le quattro copie locali non esistono più;
