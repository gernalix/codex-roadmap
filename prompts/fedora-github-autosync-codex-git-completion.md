...
4. Non reimplementare discovery, clone o pull in Python/Bash: il wrapper deve limitarsi ad autenticazione, invocazione sicura di ghorg, lock anti-run-concorrenti e logging/exit code strettamente necessari.
...
## Fase C — disciplina Git permanente per Codex
...
- Niente `git push` automatico esterno.
...
3. una run reale controllata dell'autosync, dopo aver fotografato lo stato Git dei repo non puliti, verificando che nessuno di essi venga modificato;
4. `systemctl --user` per verificare unit/timer enabled+active e prossima esecuzione;
5. test esistenti del publisher strettamente pertinenti dopo la modifica.
...
- MegaVault remoto contiene la disciplina `commit -> unico push finale -> unica verifica -> STOP`;
