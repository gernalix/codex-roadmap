...
2. Avvia UNA volta l'emulatore canonico Pixel_8a tramite helper `AGENTS.md`.
3. In UNA singola invocazione Gradle di androidTest (filtrata alle classi pertinenti), esegui:
   ...
   Non lanciare tutta `connectedDebugAndroidTest` senza filtro. Questa invocazione compila gli APK necessari: NON fare un `assembleDebug` separato.
4. Dopo i test, una sola ispezione semantica con `android_ui_summary.py` + un solo screenshot finale Now con una sessione attiva. Nessuna seconda navigazione/QA se i criteri sono già evidenti.
5. Stop emulator secondo helper. Failure => leaf fix + solo il test fallito; una sola riconferma finale del set filtrato.
