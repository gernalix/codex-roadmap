...
- `version.txt` deve essere `52` e `app/src/main/assets/datasette-lite/` deve essere presente/non vuoto;
...
# Esecuzione minima
1. Acquisisci `python3 tools/personalhub_task_lock.py acquire --prompt-id 294731`. In un unico preflight: richiedi worktree pulito e branch `main`; fai un solo fetch + `git merge --ff-only origin/main`; fissa `RUN_HEAD=origin/main=HEAD`; verifica che la baseline sopra sia antenata, `version.txt=52` e gli asset Datasette Lite siano presenti. Se no => `BLOCKED`. Nessun altro discovery Git.
