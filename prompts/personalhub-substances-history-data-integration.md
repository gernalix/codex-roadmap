...
# Goal
Substances v2 — history/data-integration phase. Make canonical History identity-stable and scalable, and remove obsolete Substances-owned authoritative DB transfer behavior so the module uses PersonalHub's single global `personalhub.db` infrastructure.

## Prerequisite
Verify the canonical intake/scheduling command semantics from the preceding Substances phase. Verify PH remains single-DB with global DatabaseVault/DatabaseGate/auto-export. If materially absent, stop BLOCKED; do not recreate prior phases.
...
Resolve preceding command/schedule files once from exact imports and follow intake/history DAO declarations directly. No module or DB-infrastructure audit. Increment `version.txt` exactly once by +1.

## History correctness/performance
...
Final output only: `PROMPT_ID`, `RESULT`, history identity/performance strategy, import wiring, legacy paths removed/retained, export semantics, tests, commit SHA.
