...
- commit minimo richiesto: `467e2e225249c9e9c9ef4c0f93a90df50f4b9b11` o successivo;
- runtime canonico: Fedora locale; Oracle VM NON è runtime;
- il codice e il test deterministico sono già sul remoto: NON reimplementare il fix;
...
- `PROMPT_ID=583214` ha almeno più esecuzioni storiche ed è il caso di verifica.
...
2. Verifica solo che `467e2e225249c9e9c9ef4c0f93a90df50f4b9b11` sia antenato di HEAD. Non rieseguire suite deterministiche già coperte dalla CI remota; se la CI di quel commit o successivo non è PASS, `BLOCKED`.
3. Esegui UNA volta il deploy canonico già documentato (`python3 deploy_runtime.py`) dal checkout pulito/sincronizzato.
4. Avvia UNA volta `codex-usage-publisher.service` oppure l'equivalente comando runtime canonico già installato, lasciando che completi il normale backfill/push. Nessun loop/retry identico.
