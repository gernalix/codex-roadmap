...
EXECUTION_CONTRACT = (
    ...
    "Do not inspect later prompts. For target-repository sync, use git fetch -q origin "
    "<branch> and, only if integration is needed, git merge --ff-only origin/<branch>; "
    "--ff-only is not a git fetch option. Reuse verified session evidence, keep exploration "
    "and tests targeted, group independent checks into one tool call when safe, avoid "
    "equivalent retries, and stop at PASS/BLOCKED/FAIL. If recovery changes tracked "
    "files in the target repository, PASS requires the necessary fix to be committed "
