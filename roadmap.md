...
- **Fedora autosync / completion guard hardening:** il precedente task ha installato e attivato ghorg/autosync e registrato i repo in MegaVault, ma il guard post-Codex usa ancora un solo CWD invece dei repo realmente toccati, riparsa globalmente `~/.codex/sessions` ogni minuto, il ramo autosync può tentare registrazione MegaVault anche dopo un errore ghorg e il publisher esporta `RESULT=PASS` come `status=UNKNOWN`. Il nuovo prompt parte direttamente da questi call-site e preserva tutto il resto.
...
1. [[prompts/fedora-github-autosync-completion-guard-hardening|fedora-github-autosync-completion-guard-hardening]] — **GPT-5.5 / medium / FAST**
...
- **Fedora completion guard hardening** resta autonomo e primo: è una correzione pre-localizzata dell'infrastruttura appena introdotta e riduce costo/false assurance per tutte le sessioni successive; non richiede riaprire i goal PersonalHub.
...
- Il fix Fedora viene **prima di tutto** perché corregge il controllo post-task e una rescansione permanente introdotti dall'ultimo task; i difetti sono già localizzati e il fix è FAST.
