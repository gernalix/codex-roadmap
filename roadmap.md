...
Finding tecnici/funzionali ancora pendenti: il toast di successo dopo un tap sui pulsanti Timer `Events` usa wording generico come `Event recorded` invece di `<titolo pulsante> added`, e deve essere coerente tra tap in-app e widget; Places non espone ancora il flusso manuale completo di check-in immediato + retroattivo; UI Composer v29 embedded/nested in Timer giudicata inutilizzabile e da sostituire con una sezione top-level dedicata; People call-overlay deep-link/race/PII. Dettagli storici: `audits/2026-09-05-personalhub-main-audit.md`.

## Ordine pendente

La roadmap contiene ora **7 goal sostanziali**. L'accorpamento resta basato sul costo Codex reale: stesso modulo/modello, file e dominio condivisi, possibilità di riusare esplorazione e test, un solo incremento versione e soprattutto una sola build/installazione/QA finale per goal. Il nuovo fix Timer resta separato perché il precedente goal Timer è già completato/archiviato e non va riaperto retroattivamente; è un intervento FAST molto localizzato da chiudere prima del più ampio goal Places.
...
- Il nuovo fix Timer `Events` viene prima perché è un bug UX molto localizzato, con starting files già pre-localizzati e costo GPT-5.5/low/FAST; chiuderlo subito evita di trascinarlo dentro i goal più ampi successivi.
