...
Finding tecnici/funzionali ancora pendenti: prima di altri lavori PersonalHub va provato il nuovo canale di consegna dell'APK finale tramite il `telegram_notify` condiviso verso `chat_id=-1004474036791`, inclusa la capacità di inviare documenti senza caption; il toast di successo dopo un tap sui pulsanti Timer `Events` usa wording generico come `Event recorded` invece di `<titolo pulsante> added`, e deve essere coerente tra tap in-app e widget; Places non espone ancora il flusso manuale completo di check-in immediato + retroattivo; UI Composer v29 embedded/nested in Timer giudicata inutilizzabile e da sostituire con una sezione top-level dedicata; People call-overlay deep-link/race/PII. Dettagli storici: `audits/2026-09-05-personalhub-main-audit.md`.

## Ordine pendente

La roadmap contiene ora **8 goal sostanziali**. Il nuovo preflight Telegram è volutamente il primo: è un prerequisito operativo piccolo e una tantum per tutti i futuri APK finali PersonalHub e non richiede build/version bump dell'app. L'accorpamento del resto resta basato sul costo Codex reale: stesso modulo/modello, file e dominio condivisi, possibilità di riusare esplorazione e test, un solo incremento versione e soprattutto una sola build/installazione/QA finale per goal. Il fix Timer resta separato perché il precedente goal Timer è già completato/archiviato e non va riaperto retroattivamente; è un intervento FAST molto localizzato da chiudere prima del più ampio goal Places.

1. [[prompts/personalhub-telegram-apk-channel-delivery-readiness|personalhub-telegram-apk-channel-delivery-readiness]] — **GPT-5.5 / low / FAST**
2. [[prompts/personalhub-timer-event-title-success-toast|personalhub-timer-event-title-success-toast]] — **GPT-5.5 / low / FAST**
3. [[prompts/personalhub-places-history-map-geofencing|personalhub-places-history-map-geofencing]] — **GPT-5.5 / medium / STANDARD** · alternativa 5.6: **GPT-5.6 Sol / medium**
4. [[prompts/personalhub-context-composer-redesign|personalhub-context-composer-redesign]] — **GPT-5.6 Sol / medium / STRICT**
5. [[prompts/personalhub-database-schema-upgrade-safety|personalhub-database-schema-upgrade-safety]] — **GPT-5.6 Sol / medium / STRICT**
6. [[prompts/personalhub-people-call-overlay-hardening|personalhub-people-call-overlay-hardening]] — **GPT-5.5 / medium / FAST** · alternativa 5.6: **GPT-5.6 Sol / medium**
7. [[prompts/personalhub-global-ui-theme-version-backup-status|personalhub-global-ui-theme-version-backup-status]] — **GPT-5.5 / medium / STANDARD** · alternativa 5.6: **GPT-5.6 Terra / medium**
8. [[prompts/personalhub-global-activity-register-safe-undo|personalhub-global-activity-register-safe-undo]] — **GPT-5.6 Sol / medium / STRICT**

## Cosa è stato accorpato e perché

- **Telegram APK delivery: preflight separato e una tantum.** Prima di consumare una sessione PersonalHub che produce un APK, verifica sul serio che il notifier Telegram condiviso — e non l'helper di `amici_fb`/project_id 1 — possa pubblicare nel nuovo canale e inviare un documento con filename controllato e senza caption. Se serve una correzione del notifier condiviso, la risolve una volta qui; non avrebbe senso ripetere lo stesso debugging dentro ogni futuro goal dell'app.
...
- Il preflight `personalhub-telegram-apk-channel-delivery-readiness` viene per primo perché `MegaVault/ai/personalhubdoc.md` rende ora obbligatoria la consegna Telegram dell'APK finale. Provare e, se necessario, riparare una sola volta il trasporto condiviso evita che un futuro goal applicativo arrivi a fine QA e rimanga bloccato soltanto sull'upload.
- Il fix Timer `Events` viene subito dopo perché è un bug UX molto localizzato, con starting files già pre-localizzati e costo GPT-5.5/low/FAST; chiuderlo subito evita di trascinarlo dentro i goal più ampi successivi.
