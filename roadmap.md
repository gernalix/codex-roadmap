...
- **Fedora completion guard — hardening finale:** il precedente fix ha corretto status terminale e fail-safe autosync, ma restano tre difetti localizzati: il CWD generale viene ancora aggiunto anche quando esistono repo espliciti dei tool call; ogni rollout viene letto/parso una seconda volta dal wrapper dopo il parser legacy; un fetch/rev-list fallito può ancora degradare erroneamente a `clean_synced`. Inoltre la precedente evoluzione dei metrics ha provocato un backfill di 498 cicli: il nuovo prompt impedisce che soli metadata derivati del guard causino altro churn storico.
...
1. [[prompts/fedora-completion-guard-final-hardening|fedora-completion-guard-final-hardening]] — **GPT-5.5 / low / FAST**
...
- **Fedora completion guard final hardening** resta autonomo e primo: è un fix pre-localizzato di pochi simboli nel publisher, necessario per evitare falsi segnali e costo permanente prima delle sessioni successive.
...
- Il fix Fedora viene **prima di tutto** perché elimina falsi `clean_synced`, una seconda lettura di ogni rollout e un falso repo CWD nel guard; è un fix FAST già localizzato.
