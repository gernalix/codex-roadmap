...
Finding tecnici/funzionali ancora pendenti: prima di altri lavori PersonalHub va provata la consegna dell'APK finale tramite il `telegram_notify` condiviso verso **la stessa destinazione predefinita che veniva usata per le precedenti notifiche Codex di progresso/stato**, senza hard-code di un chat ID diverso, inclusa la capacità di inviare documenti senza caption; il toast di successo dopo un tap sui pulsanti Timer `Events` usa wording generico come `Event recorded` invece di `<titolo pulsante> added`, e deve essere coerente tra tap in-app e widget; Substances → Prescriptions espone ancora i campi `orderEpochDay` e `prescriptionEpochDay` come valori tecnici epoch-day invece di normali date selezionabili da calendario; UI Composer v29 embedded/nested in Timer giudicata inutilizzabile e da sostituire con una sezione top-level dedicata; People call-overlay deep-link/race/PII. Dettagli storici: `audits/2026-09-05-personalhub-main-audit.md`.

## Ordine pendente

La roadmap contiene ora **8 goal sostanziali**. Il preflight Telegram è volutamente il primo: è un prerequisito operativo piccolo e una tantum per tutti i futuri APK finali PersonalHub e non richiede build/version bump dell'app. I fix Timer e Substances restano separati perché sono interventi FAST molto localizzati e non conviene trascinarli nei goal cross-module successivi. L'accorpamento del resto resta basato sul costo Codex reale: stesso modulo/modello, file e dominio condivisi, possibilità di riusare esplorazione e test, un solo incremento versione e soprattutto una sola build/installazione/QA finale per goal.
...
3. [[prompts/personalhub-substances-prescription-date-pickers|personalhub-substances-prescription-date-pickers]] — **GPT-5.5 / low / FAST**
...
- **Telegram APK delivery: preflight separato e una tantum.** Prima di consumare una sessione PersonalHub che produce un APK, verifica sul serio che il notifier Telegram condiviso — e non l'helper di `amici_fb`/project_id 1 — possa pubblicare nella sua destinazione predefinita già usata in passato per i progressi/stati Codex e inviare un documento con filename controllato e senza caption. Le notifiche Telegram di progresso/test/installazione PersonalHub restano disabilitate: questa destinazione viene riusata per la sola consegna dell'APK finale. Se serve una correzione del notifier condiviso, la risolve una volta qui; non avrebbe senso ripetere lo stesso debugging dentro ogni futuro goal dell'app.
...
- Il preflight `personalhub-telegram-apk-channel-delivery-readiness` viene per primo perché `MegaVault/ai/personalhubdoc.md` rende ora obbligatoria la consegna Telegram dell'APK finale alla destinazione predefinita condivisa già usata per i vecchi progressi/stati. Provare e, se necessario, riparare una sola volta il trasporto condiviso evita che un futuro goal applicativo arrivi a fine QA e rimanga bloccato soltanto sull'upload.
