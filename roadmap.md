...
- **Composer:** il motore Hub Context/adapters esiste già, ma `SessionEditDialog` contiene ancora `SessionContextEditor` + `HubContextLinks` e la home non ha un Composer top-level. Il redesign resta necessario e deve riusare l'infrastruttura esistente.
...
- **UI globale:** la home mostra già il `versionName` host e `DatabaseVault.autoExportStatus()` espone già tutti i dati necessari al badge backup. Restano copertura versione nei moduli, rimozione versioni feature (Substances ne mostra ancora una), badge UI e dark mode; il tema host è ancora light-only.
