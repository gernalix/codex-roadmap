...
| --: | ------ | ----------- | -------------------- | ----------- |
| 1 | [[prompts/megavault-prompt-id-registry-activation]] | **Registro degli ID dei prompt.** Mette finalmente in funzione il sistema che assegna a ogni nuovo prompt Codex un numero di 6 cifre che non è mai stato usato prima. Recupera anche tutti i numeri già usati in passato, così non potranno essere assegnati di nuovo. Serve Codex perché deve lavorare direttamente sull'archivio MegaVault presente sul tuo PC e controllare le copie locali delle vecchie conversazioni. | medium | Prompt |
