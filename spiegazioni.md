...
Sistema il bootstrap che Codex usa per lavorare su PersonalHub. Oggi MegaVault contiene un file specializzato proprio per PH ma il suo stesso controllo di validità lo considera erroneamente un Markdown vietato, generando un falso errore a ogni sessione. Il task corregge questa contraddizione senza allentare i controlli e rende più rigide le regole che evitano sprechi già osservati: un solo incremento versione per goal, meno dump UI e tentativi ripetuti, un solo ciclo compatto di test su dispositivo, APK non indovinati e messaggi di successo solo dopo un successo realmente verificato.

## 2. [[prompts/personalhub-timer-inapp-alert-cleanup|personalhub-timer-inapp-alert-cleanup]]

Completa il recente passaggio degli Alerts di Timer dalle notifiche Android ai prompt dentro l'app. Elimina il vecchio ritardo in minuti e i residui di scheduling/notifica che non servono più agli Alerts, mantenendo però intatti gli alarm delle vere sessioni temporizzate. Nello stesso punto condiviso sistema anche il ripristino di questi alarm dopo riavvio o aggiornamento dell'app. Conserva popup centrale, chiusura con X/tap esterno, link blu sottolineati, deep link, cooldown, one-time e la regola attuale sui duplicati.
