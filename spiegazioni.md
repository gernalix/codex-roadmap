...
**Alert e tag Places → Profili → Date e ore → Salute: dati → Salute: schermate → Obsidian: base → Obsidian: aggiornamenti automatici → Obsidian: tutti i moduli → Cronologia e ripristino → Esploratore dati offline → Controllo finale Google Play**
...
| 3 | [[prompts/personalhub-shared-alerts-places-tags-validation]] | **Controllare i nuovi tag e alert di Places.** Il codice è già pronto in una copia separata del progetto e non viene ancora unito alla versione principale di PersonalHub. Codex deve costruirlo sul tuo PC, verificare che l'aggiornamento dell'archivio dati non perda nulla e provarlo su un telefono Android virtuale: alert entrando o uscendo manualmente da un luogo, alert validi per categorie come “groceries”, apertura diretta dei link e collegamento opzionale a Tasker. I tag di Places devono restare completamente separati da quelli del Timer. La copia separata resta intatta perché la unirai tu più avanti. **GPT-5.6 Terra/medium + STRICT**. | medium | Prompt |
