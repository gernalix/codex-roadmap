...
## 1. [[prompts/fedora-completion-guard-final-hardening|fedora-completion-guard-final-hardening]]

Chiude tre difetti rimasti nel controllo Git che parte quando Codex termina un task. Oggi il sistema può includere anche la cartella/repository da cui è stata aperta la sessione, pur se Codex non l'ha realmente usata; inoltre ogni file di sessione viene letto due volte a ogni giro del publisher e, se un controllo remoto Git fallisce, in alcuni casi il risultato può ancora finire erroneamente come `clean_synced`. Il fix usa il CWD soltanto come vero fallback, raccoglie i repo durante l'unica lettura già necessaria del file, tratta gli errori Git come stato non verificabile e impedisce che soli metadata tecnici del guard provochino un'altra ripubblicazione in massa di centinaia di vecchi cicli.
