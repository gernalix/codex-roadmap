...
| --: | --- | --- | --- | --- |
| 1 | [[prompts/personalhub-main-baseline-local-verification]] | **Cosa fa:** verifica esclusivamente sul vecchio AVD persistente che il repair Timer ora eseguito prima della UI impedisca la perdita `tagSessions: 1 -> 0`. Codice e diagnostico DB read-only sono già stati aggiunti in chat. **Ottimizzazione:** una sola build, package Android esplicito, niente logcat/smoke/test già PASS; se il problema resta, Codex esegue un solo diagnostico automatico e si ferma senza fare discovery o fix. | low | Prompt |
