...
| 3 | [[prompts/personalhub-shared-alerts-places-tags-validation]] | **Chiudere i nuovi tag e alert di Places.** Il lavoro è in una copia temporanea separata di PersonalHub. Codex deve prima unirvi gli ultimi cambiamenti della versione principale, costruire e provare tutto sul PC e su un telefono Android virtuale, controllare che l’aggiornamento dell’archivio dati non perda nulla e verificare alert per luogo o categoria, check-in/check-out, link diretti e Tasker opzionale. Se tutto passa, deve unire subito il lavoro nella versione principale e cancellare la copia temporanea, così resta solo `main`. I tag di Places devono restare completamente separati da quelli del Timer. **GPT-5.6 Terra/medium + STRICT**. | medium | Prompt |
