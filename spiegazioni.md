...
| --: | --- | --- | --- | --- |
| 1 | [[prompts/personalhub-main-baseline-local-verification]] | **Cosa fa:** in un solo pass locale verifica sul `main` già mergiato sia il repair Timer sul vecchio stato persistente sia i nuovi confini capsule/Home-Timer-WordPulse. **Perché:** sostituisce due prompt di verifica e riusa un solo fetch, avvio emulatore, build/install e smoke. Il benchmark della façade emulatore non viene ripetuto perché è già PASS. | low | Prompt |
