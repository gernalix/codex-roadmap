...
| --: | ------ | ----------- | -------------------- | ----------- |
| 1 | [[prompts/codex-usage-verification-status-deploy]] | Il primo run si è fermato su un falso gate d'import (`codex_monitor` non visibile al `unittest` diretto). Il repo remoto ora rende i test mirati source-layout-safe e documenta il verifier canonico; restano solo test/deploy Fedora, un publisher run e readback locale. **GPT-5.6 Luna/low**. | low | Prompt |
