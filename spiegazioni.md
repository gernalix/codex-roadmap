...
| --: | --- | --- | --- | --- |
| 1 | [[prompts/github-autosync-local-audit-verification]] | Il fix remoto è già scritto: gli 11 repo invariati vengono auditati localmente, i commit locali ahead possono essere pushati, il clone rispetta il worktree canonico e dry-run/upstream non fanno fetch inutili. Codex deve solo pullare, eseguire i test e fare una singola verifica del servizio Fedora, correggendo esclusivamente failure concrete. | low | Prompt |
...
| 6 | [[prompts/personalhub-ui-people-final-hardening]] | Home export status, host/Timer dark mode e i fix call-overlay People sono già implementati; restano compile/test, Soldi system theme, label/footer versioni e una sola QA finale. | low | Prompt |
