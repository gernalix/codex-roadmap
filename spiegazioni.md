...
Automatizza la parte ripetitiva di Git su Fedora senza affidare a uno script il potere di pubblicare modifiche. Installa e configura `ghorg` perché tutti i repository GitHub di `gernalix` esistano sotto `/home/daniele/projects`: un repo nuovo viene clonato automaticamente, mentre un repo già presente viene aggiornato soltanto se non contiene lavoro locale o commit ancora da pushare. Un timer utente esegue il controllo ogni 5 minuti e non elimina copie locali se un repo sparisce dal remoto. Codex resta invece responsabile dei propri commit e push: dopo un task deve fare un solo push finale per repo, verificarlo una volta e fermarsi. Il publisher Codex già esistente viene usato anche come rete di sicurezza passiva: quando rileva il vero `task_complete`, registra se Codex ha lasciato modifiche non committate, commit non pushati o divergenze, senza correggerle automaticamente e senza creare un secondo watcher o notifiche Telegram.
