...
`roadmap_finish.py` resta compatibile ed equivale a `PASS`. Il comando non modifica più il DB o Git locale: crea una GitHub Issue immutabile `[roadmap-mutation] terminal-<PROMPT_ID>`. **Questa Issue non cambia subito lo stato del prompt**: registra una `terminal_request`, mentre il prompt resta `running` in roadmap e in `spiegazioni.md`. Di norma una successiva `usage_execution` terminale di `codex-usage` effettua la transizione reale a `completed`, `failed`, `blocked`, `cancelled` o `unknown`. In alternativa, una conferma umana esplicita proveniente dal control surface Workflowy usa `human_execution`: può chiudere un prompt `running`, registra source `workflowy-human`, elimina l'eventuale terminal request automatica pendente e lascia un audit dedicato. Per ogni PROMPT_ID esiste una sola chiave terminale; un retry identico è idempotente, mentre un esito terminale diverso viene rifiutato.
