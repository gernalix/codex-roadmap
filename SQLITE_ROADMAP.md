...
Per un **nuovo** prompt, la `register` non può essere il primo passo: ChatGPT deve prima ottenere un ID dal registry canonico MegaVault. Da remoto crea una Issue immutabile `[prompt-id-command] <request_id>` in `gernalix/MegaVault` con `command=allocate` nel body JSON. L'allocatore MegaVault registra atomicamente `request_id -> PROMPT_ID` in `megavault.sqlite:prompt_id_allocation_requests`: un retry con gli stessi parametri restituisce lo stesso PROMPT_ID, mentre gli stessi `request_id` con `source/project_id/parent_prompt_id` differenti vengono rifiutati. Le receipt JSON non sono l'autorità dell'idempotenza. Solo dopo `status=allocated` ChatGPT invia la mutation `register`. Dopo che il single writer ha materializzato il file canonico, ChatGPT invia un secondo comando Issue `materialize` con il `content_url` esatto del file su `codex-roadmap/main` e attende `status=materialized`. Il vecchio `.github/prompt-id-request.json` non è più un trasporto operativo; `.github/prompt-id-response.json` è soltanto una proiezione di compatibilità dell'ultimo risultato.
