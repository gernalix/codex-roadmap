...
class RoadmapGuardTests(unittest.TestCase):
    ...
    def test_select_execution_pack_contains_prompt_and_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("git fetch -q origin <branch>", selected["execution_contract"])
            self.assertIn("git merge --ff-only origin/<branch>", selected["execution_contract"])
            self.assertIn("--ff-only is not a git fetch option", selected["execution_contract"])
